...
export const CreativeAssistant: React.FC<CreativeAssistantProps> = ({ 
    ...
    // Handle Asset Picked from Timeline/Workspace
    useEffect(() => {
        if (pickedAsset) {
            insertMention({ id: pickedAsset.id, name: pickedAsset.name });
        }
    }, [pickedAsset]);
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Detect if user typed '@' manually to still support keyboard usage
    ...
    const insertMention = (asset: {id: string, name: string}) => {
        // Check if we are completing a manual type query
        if (mentionQuery !== null) {
            ...
            // Just append it (clicked from UI)
            setInput(prev => {
                const space = prev.length > 0 && !prev.endsWith(' ') ? ' ' : '';
                return `${prev}${space}@[${asset.name}](${asset.id}) `;
            });
        }
    ...
    return (
        <div className="flex flex-col h-full bg-neutral-900 relative">
            ...
            {/* Mention Popup (Fallback/Keyboard) */}
